<?php
    ...
    } $query = "SELECT `idd`,`part-number`,`performer`,`date`,`upper`,`lower` FROM `attribute-gauge-r&r-study` ORDER BY `date` DESC";
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        $displayField .= "<div class='alert alert-dark text-left' role='alert'><a href='procedure-5.php?id=".$row['idd']."'>".$i.") Part Number : ".$row['part-number']."<br>   Performer : ".$row['performer']."<br>   Date : ".$row['date']."</a></div>";
        $i++;
    
    } if($displayField == "") {
        
        $displayField = "No Attribute R&amp;R studies have been performed yet.";
